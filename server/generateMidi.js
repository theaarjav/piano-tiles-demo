...
const midiData = fs.readFileSync('./sunflower_piano_edited.mid');
...
pianoTrack.notes.forEach(async (note) => {
  ...
  newObj.push({
    ...
    tiles: Math.min((parseInt(note.duration / 0.333)) + 1, 4)
  })
});
// console.log(newObj);
...
function organizeNotes(notesData) {
  ...
  for (const noteData of notesData) {
    ...
    organizedNotes[startTime].push({
      ...
      tiles: Math.max(1, (parseInt(noteData.duration / 0.333)))
    });
  }
  for (let i = 1; i <= 4; i++) {
    organizedNotes[(parseFloat(startEnd) + i * 0.333).toString()] = 'blank_tiles'
  }
...
export const printNotes = (io) => {
  if (io) {
    ...
    const currentTimeInterval = parseFloat(currTime).toFixed(2);
    const endTimeInterval = (parseFloat(currTime) + 0.333).toFixed(2);
    let highestNote = null;
    // io.emit("GAME_START_TO_CLIENT");
    if (result[keysArray[currentIndex]] == 'blank_tiles' || (parseFloat(keysArray[currentIndex]) < currentTimeInterval ||
      ...
      // setTimeout(() => printNotes( io), 333.33); // Set the timeout to 333.33 milliseconds for the next interval
      // return;
    }
    else {
      ...
      generateRow(io, highestNote.noteInfo.tiles);
    }
    time+=0.333;
...
fs.writeFileSync('./sunflower.js', 'export const result=' + JSON.stringify(result))